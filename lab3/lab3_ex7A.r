...
event_list <- c(rexp(1, arrival_rate), rexp(1, arrival_rate), Inf)
	...
	if (next_event_type == 3) { # departure
		if (num_in_queue_1 == 0) { # No clients in queue 1
			...
			if (num_in_queue_2 == 0) { # No clients in queue 2
				...
				# Schedule departure
				event_list[3] <- time + rexp(1, service_rate[2])
			}
			...
			# Schedule departure
			event_list[3] <- time + rexp(1, service_rate[1])
		}
		...
		if (next_event_type == 1) { # Client arrived to queue 1
			...
			# Schedule next arrival
			event_list[2] <- time + rexp(1, arrival_rate)
